...
%function compute_mean_3D(Draw_Models, quantity, fig_num, gen_elem_list, gen_avg_data)
...
Models      = {'DNS'; 'LDF'; 'LProj'; 'EFROM'; 'EFProj'; 'POD'};
%if nargin==0
...
for i=1:length(Models),...
        cmp_model = [cmp_model,num2str(cmp_res(i))];
end
% ---- set time range
n_per_data = 4000;
n_per_start= 1;
...
for i=1:length(Models) % ---- model
    if cmp_res(i)==1
        if i==1 % ---- for DNS
            ...
            fprintf(1, 'DNS: \n');
        elseif i==2 % ---- for LDF
            str1 = 'r-o'; str2=[str2;'LDF-POD-ROM  ']; tt=tt+1;
            nameq='LDF-ROM';
            Dir_load = ['Matrices/r',num2str(q_dim),'/connective_matrix145_193_17_'];
            Cur_name = [num2str(n_per_data), 'LDF_0.22_a'];
            Loadname = [Dir_load, Cur_name, '.mat'];
            load(Loadname, 'a');
            a        = a(:, T_set);
            fprintf(1, 'LDF: \n');
        elseif i==3 % ---- for LProj
            str1 = 'g-x'; str2=[str2;'LProj-POD-ROM']; tt=tt+1;
            r1        = 2;
            nameq='LProj-ROM';
            Dir_load = ['Matrices/r', num2str(q_dim),'/connective_matrix145_193_17_'];
            Cur_name = [num2str(n_per_data), '_', 'r1_', num2str(r1), '_a']; % change this
            Loadname = [Dir_load, Cur_name, '.mat'];
            load(Loadname, 'a');
            a        = a(:, T_set);
            fprintf(1, 'LProj: \n');
        elseif i==4 % ---- for EFROM
            str1 = 'c-+'; str2=[str2;'EF-POD-ROM ']; tt=tt+1;
            nameq='EF-ROM';
            de        = 0.0014;
            Dir_load = ['Matrices/r',num2str(q_dim),'/connective_matrix145_193_17_',...
                num2str(n_per_data),'EFROM_'];
            Cur_name = [num2str(de),'_a'];
            Loadname = [Dir_load, Cur_name, '.mat'];
            load(Loadname, 'a');
            a        = a(:, T_set);
            fprintf(1, 'EFROM : \n');
        elseif i==5 % ---- for EFProj
            str1 = 'm-s'; str2=[str2;'EFProj-ROM ']; tt=tt+1;
            nameq='EF-Proj-ROM';
            Dir_load = ['Matrices/r',num2str(q_dim),'/connective_matrix145_193_17_',...
                num2str(n_per_data),'EFProj_4_a'];
        ...
        savename   = ['Matrices/AVG/r',num2str(q_dim),'Avg_yzt_',nameq,'_',quantity];   % save data for avg
        
        if gen_avg_data == 1
            [N, M]     = size(a);
            snaps      = 1:skip:M;
            q_txyz           = zeros(length(snaps), x_num_pts, y_num_pts, z_num_pts);
            q_yavg           = zeros(length(snaps), y_num_pts, z_num_pts);
            q_yavg_zavg      = zeros(length(snaps), y_num_pts);
            q_yavg_zavg_tavg = zeros(y_num_pts,1);          
            % ---- interpolation
            for k = 1:z_num_pts
...
%%-------------The following is zhu's origin code            
%             disp('take x average now')
...
%%------------------- take the yzt avg            
            disp('take y average now')
            % ---- take avg in y direction
            for k = 1:z_num_pts
                for t_m = 1:length(snaps);
                    for j = 1: x_num_pts % fix a y
                        q_yavg(t_m, j, k) = trapz(q_txyz(t_m,j,:,k))/(y_num_pts-1) ; % <u>_y
                    end
                end
            end
                        disp('take z average now')
            ...
            for j = 1:x_num_pts
                for t_m = 1:length(snaps)
                    q_yavg_zavg(t_m,j) = trapz(q_yavg(t_m,j,:))/(z_num_pts-1); %<u>_y,z
                end
            end
            disp('take t average now')
            % ---- take avg in time
            for j = 1:x_num_pts
                q_yavg_zavg_tavg(j,1) = trapz(q_yavg_zavg(:,j))/( length(snaps)-1 ); %<u>_y,z,t
            end
            
            save(savename, 'q_txyz', 'q_yavg', 'q_yavg_zavg', 'q_yavg_zavg_tavg');
        ...
        figure(fig_num)
        legend_idx(1,tt)= plot(y_pts, q_yavg_zavg_tavg,str1,'linewidth',1.1);
        hold on
    end
    if i==length(Models)
        figure(fig_num)
        ...
        figure_avg = ['Matrices/AVG/figs/r',num2str(q_dim),'Avg_yzt_',quantity, cmp_model];
        xlabel('y'); ylabel(['<',quantity,'>_{y,z,t}'])
        set(gca,'FontSize',11)
