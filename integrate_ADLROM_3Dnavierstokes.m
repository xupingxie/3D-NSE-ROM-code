function [a,Mr,DF_POD,cod_ED] = integrate_ADLROM_3Dnavierstokes(a0,...
    t_range,mu,de,nu,v_noise)
%============================================================
% POD-G-ROM
%============================================================
global PMP filename
...
S=S11+S22+S33;
Mr=POD_u'*M*POD_u+POD_v'*M*POD_v+POD_w'*M*POD_w;                 %---------------compute DF operator
Sr=POD_u'*S*POD_u+POD_v'*S*POD_v+POD_w'*S*POD_w;
DF_POD=Mr+de^2*Sr;

clear S11 S12 S13 S22 S23 S33 S M
...
B =  2*mu*(Suxux') + mu*(Suyuy' + Svxuy' + Suzuz' + Swxuz') +...
     mu*(Svxvx' + Suyvx') + 2*mu*(Svyvy') + mu*(Svzvz' + Swyvz') +...
     mu*(Swxwx' + Suzwx' + Swywy' + Svzwy') + 2*mu*(Swzwz') ;

B_AD= cupux + pucux' + cvpuy + pvcuy' + cwpuz + pwcuz' + ...
      cupvx + pucvx' + cvpvy + pvcvy' + cwpvz + pwcvz' + ...
      cupwx + pucwx' + cvpwy + pvcwy' + cwpwz + pwcwz';


D=B_AD;
%E=Non_constant;

filename4 = [filename, '_ABCD.mat'];
save(filename4, 'A','B','C','D')
...
for i_time = 1:length(t_range)-1
    ...
    for n=1:n_sub
        ...
        Nh=zeros(r,1);
        for i=1:r
            Nh(i) = a_ADL'*C(i).mat*a_ADL;
        end
        C_bar=DF_POD\Nh;
        Nh_bar = Mr*C_bar;
        rhs = -A - B*a_c-D*a_c-Nh_bar;       
        %Non = Non_constant+D*a_ADL+Nh;
        %Nh_bar=DF_POD\Non;
